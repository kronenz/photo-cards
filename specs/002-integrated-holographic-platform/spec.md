...
포토카드 수집에 초점을 맞춘 메인 화면에서 사용자의 컬렉션, 커뮤니티 피드, KBO 팀 섹션을 통합하여 한 곳에서 모든 활동을 관리할 수 있다.
...
### User Story 3 - KBO Fan Culture Integration (Priority: P2)

한국 야구 문화가 반영된 특별한 경험을 통해 KBO 팬으로서의 정체성을 표현하고, 구단별 커뮤니티에서 동료 팬들과 교류할 수 있다.

**Why this priority**: 차별화 요소이지만 MVP 핵심 기능(카드 + 메인)이 먼저 완성되어야 의미 있음.

**Independent Test**: 사용자가 자신의 응원 구단(예: LG 트윈스)을 선택하고, 메인 화면이 팀 컬러(보라색)로 변경되며, KBO 팬 등급(예: 외야석 팬 → 응원단 멤버)이 활동에 따라 업그레이드되는 것을 확인할 수 있으면 성공.

**Acceptance Scenarios**:

1. **Given** 사용자가 설정에서 응원 구단을 선택했을 때, **When** 메인 화면으로 돌아오면, **Then** 시스템은 해당 팀의 공식 컬러(예: LG 보라색, 두산 네이비)를 헤더와 액센트에 적용한다
2. **Given** 사용자가 카드를 생성하고 커뮤니티에 참여할 때, **When** 활동 점수가 임계값을 넘으면, **Then** 시스템은 KBO 팬 등급을 업데이트하고 새 등급 해금 알림을 표시한다 (야구 입문자 → 외야석 팬 → 응원단 멤버 → 시즌권 홀더 → 구단 레전드)
3. **Given** KBO 정규 시즌 중일 때, **When** 사용자가 메인 화면을 방문하면, **Then** 시스템은 응원 구단의 오늘 경기 정보(상대팀, 시간, 현재 스코어)를 배너로 표시한다
...
- **테마 우선순위**: 사용자가 KBO 팀 테마와 다크모드를 동시에 설정했을 때 어떤 스타일이 우선하는가?
...
- **FR-012**: 시스템은 사용자 활동 점수에 따라 KBO 팬 등급(5단계)을 자동 업데이트해야 한다
- **FR-013**: 시스템은 KBO 정규 시즌 중 응원 구단의 오늘 경기 정보를 메인 화면 배너로 표시해야 한다
...
- **UnifiedUser**: 통합 사용자 모델
  ...
  - KBO 팬: fanLevel (5단계), favoriteTeam
...
- **SC-007**: KBO 팬 등급이 활동 점수 임계값 도달 시 즉시(1초 이내) 업데이트된다
...
- **테마 우선순위**: KBO 팀 테마가 다크모드보다 우선하며, 사용자가 팀을 선택하지 않으면 기본 테마 사용
...
- **KBO API**: 실시간 경기 정보는 KBO 공식 API 또는 크롤링으로 수집 (법적 검토 필요)
...
- **KBO API**: 실시간 경기 정보, 구단 데이터 (외부 API 또는 크롤링)
